...
<dialog bind:this={dialog} class="detail-dialog" data-testid="detail-panel" aria-labelledby="detail-title" oncancel={onClose} onclick={e => { if (e.target === dialog) onClose(); }} onkeydown={e => { if (e.key === 'Escape') onClose(); }}>
  <div class="detail-shell">
    ...
    <div class="detail-body">
      ...
      <div class="deadline-hero"><span><Clock3 size={14}/> 报名截止 · 北京时间</span><strong>{row.status === 'cancelled' ? '项目已取消' : row.remainingMs !== null && row.remainingMs > 0 ? formatRemaining(row.remainingMs) : formatPoint(p.applicationEnd)}</strong>{#if row.remainingMs !== null}<small>{formatPoint(p.applicationEnd)}</small>{/if}</div>
      ...
      <section class="detail-section"><h3>来源检查</h3>{#each row.sources as source}<div class="source-check"><a href={source.url} target="_blank" rel="noreferrer">{source.title || source.url}<ArrowUpRight size={12}/></a><p>最后成功检查：{formatTimestamp(source.lastSuccessAt)}</p>{#if sourceStale(source, clock.now)}<span class="warning-text">{source.lastSuccessAt ? '超过 36 小时未成功检查，信息可能已变动' : '尚未成功检查此来源，请先阅读官方通知'}</span>{/if}{#if source.error}<p class="warning-text">本次检查异常：{source.error}</p>{/if}</div>{/each}</section>
